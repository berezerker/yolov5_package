...
def detect_image(weights, image_url, img_size, conf_thres, iou_thres, class_name):
    ...
    yolov5_tflite_obj = Yolov5Tflite(
        weights, img_size, conf_thres, iou_thres, class_name
    )
    ...
    if len(result_boxes) > 0:
        result_boxes = scale_coords(
            size, np.array(result_boxes), (original_size[1], original_size[0])
        )
        ...
        for i, r in enumerate(result_boxes):
            ...
            org = (int(r[0]), int(r[1]))
            cv2.rectangle(
                img, (int(r[0]), int(r[1])), (int(r[2]), int(r[3])), (255, 0, 0), 1
            )
            cv2.putText(
                img,
...
def main():
    ...
    parser.add_argument(
        "-i", "--img_path", type=str, default="image_test.jpeg", help="image path"
    )
    parser.add_argument("--img_size", type=int, default=416, help="image size")
    parser.add_argument(
        "--conf_thres", type=float, default=0.25, help="object confidence threshold"
    )
    parser.add_argument(
    ...
    opt.weights = pkg_resources.resource_filename(
        "yolov5_hse", "model_float16_quant.tflite"
    )
